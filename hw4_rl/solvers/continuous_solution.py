...
class DiscretizedSolver:
    ...
    def get_discrete_state_probabilities(self, continuous_state: np.ndarray) -> List[Tuple[int, float]]:
        ...
        if self._mode == "nn":
            ...
            idx = np.floor((cs - self.state_lower_bound) / self.bin_sizes).astype(int)
            idx = np.clip(idx, 0, self._num_bins - 1)
            state_index = idx[0] * self._num_bins + idx[1]
    ...
    def _value_iteration(self, max_iterations: int, min_iter: int, eval_sample_size: int) -> None:
        ...
        for i in range(max_iterations):
            ...
            V = self.solver.get_value_function().copy()
            ...
            for s in range(self.solver.num_states):
                for a in range(self.solver.num_actions):
                    Q[s, a] = np.dot(T[s, a, :], R[s, a, :] + self.gamma * V)

            # Update value function and policy
            V_new = np.max(Q, axis=1)
            value_diff = np.max(np.abs(V_new - V))
            V = V_new
            ...
            policy = np.eye(self.solver.num_actions)[a_star]
